...
std::unordered_map<std::string, size_t> split_str(std::string& s) {
    ...
    while(next_equation_mark != s.size()) {
        ...
        //std::cout << s.substr(question_mark_pos + 1, next_equation_mark - question_mark_pos - 1) << " " << s.substr(next_equation_mark + 1, next_ampersanda - next_equation_mark - 1) << std::endl;
        question_mark_pos = next_ampersanda;
        //std::cout << next_equation_mark << ' ' << next_ampersanda << ' ' << s.size() << std::endl;
...
http::message_generator handle_request(beast::string_view doc_root, http::request<Body, http::basic_fields<Allocator>>&& req
    ...
    if(input["method"] == 0) {
        server->Buy(input["itemId"], input["itemCount"], input["sessionId"]);
    }
...
public:
    ...
    void on_read(beast::error_code ec, std::size_t bytes_transferred) {
        ...
        if(ec)
            return fail(ec, "read");
        send_response(handle_request(*doc_root_, std::move(req_), server, server_mutex));
    }
